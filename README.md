...
Welle-39 LLM Sub-Funktion Extension: **HeyLou als Sub-Funktion in DeepSeek Function-Calling**.
...
DeepSeek Function-Calling: Tool-Declaration als JSON-Schema, Modell ruft `functionCall` → unsere Extension routet zu HeyLou-API-Backend.
...
- `DF_HEYLOU_DEEPSEEK_EXT_ENABLED=false` → Mock-Responses (HeyLou Travel-Knowledge-Graph synthetisch)
- `DF_HEYLOU_DEEPSEEK_EXT_ENABLED=true` + `PHRONESIS_TICKET` + `DEEPSEEK_API_KEY` → Real-Mode
