"""Tests for DeepSeekExtension dispatch [CRUX-MK]."""
...
from src.deepseek_extension import (
    ExtensionProvenance,
    ExtensionResponse,
    DeepSeekExtension,
    K0_FUNCTIONS,
...
def test_book_direct_is_k0_relevant_in_sandbox_safe():
    ...
    assert resp.data["booking_id"].startswith("MOCK-deepseek-")
...
def test_real_mode_requires_phronesis_ticket():
    ...
    assert ext.is_real_mode() is False
    os.environ.pop("DF_HEYLOU_DEEPSEEK_EXT_ENABLED", None)
...
def test_provenance_envelope_has_required_fields():
    ...
    assert p.extension_id == "df-heylou-deepseek-extension"
