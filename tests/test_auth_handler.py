"""Tests for DeepSeekAuthHandler (K13 PAV) [CRUX-MK]."""
...
from src.auth_handler import AuthSource, DeepSeekAuthHandler, DeepSeekCredentials
...
def test_real_mode_without_api_key_returns_missing():
    os.environ["DF_HEYLOU_DEEPSEEK_EXT_ENABLED"] = "true"
    handler = DeepSeekAuthHandler()
...
def test_real_mode_with_api_key_validates():
    os.environ["DF_HEYLOU_DEEPSEEK_EXT_ENABLED"] = "true"
    os.environ["DEEPSEEK_API_KEY"] = "fake-real-gemini-key-abc123"
    handler = DeepSeekAuthHandler()
    creds = handler.get_credentials()
...
def test_phronesis_ticket_verification():
    handler = DeepSeekAuthHandler(sandbox_mode=False)
...
def test_k0_action_allowed_in_sandbox():
    handler = DeepSeekAuthHandler(sandbox_mode=True)
