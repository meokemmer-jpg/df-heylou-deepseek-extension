...
Phasen:
1. auth   -> DeepSeekAuthHandler.get_credentials()
...
DF_ID = "df-heylou-deepseek-extension"
PROVIDER = "gemini"
...
class DeepSeekAdapterOrchestrator:
    DF_ID = DF_ID
    PROVIDER = PROVIDER

    def __init__(self, tenant_id: Optional[str] = None):
        self.tenant_id = tenant_id or os.environ.get(
            "DF_HEYLOU_DEEPSEEK_TENANT_ID", "hildesheim"
        )
        self.sandbox_mode = (
            os.environ.get("DF_HEYLOU_DEEPSEEK_EXT_ENABLED", "false") != "true"
        )
        from src.deepseek_extension import DeepSeekExtension
        from src.auth_handler import DeepSeekAuthHandler
        from src.audit_logger import AuditLogger
        from src.function_definitions import get_function_names

        self.extension = DeepSeekExtension(sandbox_mode=self.sandbox_mode)
        self.auth = DeepSeekAuthHandler(sandbox_mode=self.sandbox_mode)
...
def main():
    ...
    orch = DeepSeekAdapterOrchestrator()
    report = orch.run()
    print(
...
if __name__ == "__main__":  # K16+K11-FOUNDATION-WIRED [CRUX-MK]
    ...
    raise SystemExit(_rg("df-heylou-deepseek-extension", main))   # K14+K16+K15+K11 echt