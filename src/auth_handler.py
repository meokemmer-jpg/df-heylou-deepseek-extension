"""DeepSeek-Auth-Handler fuer HeyLou-Extension [CRUX-MK].

Welle-39. User-OAuth (DEEPSEEK_API_KEY) + Service-Identity-Token (DF_HEYLOU_DEEPSEEK_HMAC_SECRET).
...
class DeepSeekAuthHandler:
    """Gemini-Auth-Handler.

    Public-API:
    - get_credentials(tenant_id) -> DeepSeekCredentials
    - validate(creds) -> bool
    ...
    def __init__(self, sandbox_mode: bool | None = None):
        if sandbox_mode is None:
            sandbox_mode = os.environ.get("DF_HEYLOU_DEEPSEEK_EXT_ENABLED", "false") != "true"
        self.sandbox_mode = sandbox_mode

    def _now_iso(self) -> str:
        return datetime.now(timezone.utc).isoformat()

    def get_credentials(self, tenant_id: str = "hildesheim") -> DeepSeekCredentials:
        ...
        if self.sandbox_mode:
            return DeepSeekCredentials(
                api_key=api_key or "sandbox-fake-deepseek-key",
    ...
    def validate(self, creds: DeepSeekCredentials) -> bool:
